...
For a migration-based workflow instead, `pnpm db:generate` writes the SQL and `pnpm db:migrate` applies it.

Then seed the three worlds:

```bash
pnpm seed:all
```

The seeds run in dependency order — each world before the extra locations that belong to it — and can also be run individually:

| Script                    | Seeds                                            |
| ------------------------- | ------------------------------------------------ |
| `pnpm seed:treigthe`      | Tréigthe (fantasy), Talamh Liath, Cathair Luaith |
| `pnpm seed:baile-fola`    | Additional Tréigthe city                         |
| `pnpm seed:drift`         | The Drift (sci-fi)                               |
| `pnpm seed:tetherport`    | Additional Drift location                        |
| `pnpm seed:neon-warszawa` | Neon Warszawa 2087 (cyberpunk)                   |
...
| `pnpm build:start` | Build, then serve — what the e2e suite runs against |
| `pnpm lint` / `pnpm eslint:fix` | ESLint |
| `pnpm stylelint` / `pnpm stylelint:fix` | Stylelint over CSS |
| `pnpm test:unit` | Vitest |
| `pnpm test:e2e:run` / `pnpm test:e2e:open` | Cypress |
| `pnpm db:push` / `db:generate` / `db:migrate` / `db:studio` | Drizzle Kit |
| `pnpm seed:all` | Seed every world |
